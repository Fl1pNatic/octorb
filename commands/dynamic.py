...
class dynamic(commands.Cog):
    ...
    async def quickcommand(self, ctx: commands.Context):
        if ctx.invoked_subcommand is None:
            await ctx.reply("Use quickcommand create [name, message], delete [name], or list.")
    ...
    async def qcreate(self, ctx: commands.Context, command_name: str, *, message: str):
        ...
        if currentAmount[0] != 0:
            cursor.execute("UPDATE quickCommands SET output = %s WHERE serverId = %s AND command = %s",
                           (message, ctx.guild.id, command_name))
        else:
            cursor.execute("INSERT INTO quickCommands VALUES ( %s, %s, %s )",
    ...
    async def gallery(self, ctx: commands.Context):
        if ctx.invoked_subcommand is not None:
            return
        await ctx.reply("Use gallery show [media id], gallery add [media], gallery count, or gallery delete [media id].")
        return
